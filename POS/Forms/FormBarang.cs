    ...
    public partial class FormBarang : Form
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                DataRow[] rowHarga = datasetPOS1.tbl_harga.Select(String.Format("barang_id = {0} AND _default_ = {1}",
                    cmbNamaBarang.SelectedValue.ToString(), true));
                numHargaModal.Value = Convert.ToDecimal(rowHarga[0]["harga_model"]);
                numHargaJual.Value = Convert.ToDecimal(rowHarga[0]["harga_jual"]);

                DataRow[] rowStok = datasetPOS1.tbl_stok.Select(String.Format("barang_id = {0}",
                    cmbNamaBarang.SelectedValue.ToString()));
                numStok.Value = Convert.ToDecimal(rowStok[0]["stok"]);
        ...
        private void tsbSimpan_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    else
                    {
                        ...
                        if (cmbKategori.SelectedValue == null)
                        {
                            ...
                            else
                            {
                                ...
                                //MessageBox.Show(datasetPOS1.tbl_kategori.Rows[datasetPOS1.tbl_kategori.Rows.Count - 1]["kategori_id"].ToString());
                        ...
                        if (cmbSupplier.SelectedValue == null)
                        {
                            ...
                            else
                            {
                                ...
                                adapterSupplier.Insert(supplierID,row["nama_supplier"].ToString(), null, null, null, true, null);
                            }
                        }
                ...
                else
                {
                    ...
                    if(numStok.Value != (Int32)rowView["stok"])
                    {
                        var status = MessageBox.Show("Merubah stok dari form barang tidak akan tercatat dalam database\n" + 
                            "Disarankan menggunakan form retur untuk menyesuaikan stok barang\n" +
                            "agar tercatat dalam database, Apakah akan tetap mengupdate stok?","Ubah Data",MessageBoxButtons.YesNo);
        ...
        {
            MessageBox.Show(incrementLastIDFromTable(datasetPOS1.tbl_kategori,"kategori_id").ToString());
        }
